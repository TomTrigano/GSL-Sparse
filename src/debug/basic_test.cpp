...
int main(int argc, char *argv[])
{
	...
	gsl_vector *beta_out=gsl_vector_calloc(N);
	...
	// Test LASSO
	{
	  LASSO_LARS_solver Ls(A,y,"LASSO",maxIters,lambdaStop,resStop,1);
	  std::cout << "Solver initialized..." << std::endl;
	  Ls.SolveLASSO();
	  Ls.GetRegressor(beta_out);
	  std::cout << "beta = [";
	  for(k=0;k<beta_out->size;++k)
	    std::cout << gsl_vector_get(beta_out,k) << " " ;
	  std::cout << "]^T" << std::endl << std::endl ;
	}
	// Test LARS
	{
	  LASSO_LARS_solver Ls(A,y,"LARS",maxIters,lambdaStop,resStop,1);
	  std::cout << "Solver initialized..." << std::endl;
	  Ls.SolveLASSO();
	  Ls.GetRegressor(beta_out);
	  std::cout << "beta = [";
	...
	// Test NNLARS
	{
	  LASSO_LARS_solver Ls(A,y,"NNLARS",maxIters,lambdaStop,resStop,1);
	  std::cout << "Solver initialized..." << std::endl;
	  Ls.SolveLASSO();
	  Ls.GetRegressor(beta_out);
	  std::cout << "beta = [";
